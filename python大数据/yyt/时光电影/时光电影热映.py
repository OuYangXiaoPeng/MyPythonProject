...
for current_page in range(1, pages_to_scrape + 1):
    ...
    for idx, movie in enumerate(movies, 1):
        try:
            ...
            print(f"✓ 成功爬取: 《{title}》 (第{current_page}页第{idx}部)")
    ...
    if current_page < pages_to_scrape:
        try:
            ...
            print(f"翻页到第{current_page + 1}页失败: {str(e)}")
            break
...
if movie_data:
    ...
    df.to_csv('时光电影top.csv', index=False, encoding='utf-8-sig')
    print(f"\n✅ 成功爬取 {len(movie_data)} 条数据，已保存到 时光电影top.csv")
else:
    print("\n⚠️ 没有爬取到任何数据")
