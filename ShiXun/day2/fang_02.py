...
#此处可以修改模式'a'为'w+'
    ...
    # 用csv.writer()函数创建一个writer对象。dialect='excel',dialect，编码风格，默认为excel的风格，也就是用逗号(，）分隔,可以省略
...
while unit_loop:
    ...
    #例如：unit_rooms={3:{301:[1,80],302:[1,80],303:[2,90],304:[2,90]}}
    for floor in range(int(start_floor) + 1, int(end_floor) + 1):
    #遍历除初始楼层外的其他楼层
        floor_rooms = {}
        #每个楼层都建立一个字典
        for i in range(len(start_floor_rooms)):
        #遍历每层有多少个房间，例如：这里是3，即执行for i in range 3 的循环
